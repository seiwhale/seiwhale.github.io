...
《Effective JavaScript》P11：当 + 用在连接字符串时，当一个对象既有 `toString` 方法又有 `valueOf` 方法时候，`JS` 通过盲目使用 `valueOf` 方法来解决这种含糊。
...
假设现在有一个数列，你希望更新它的每一项（`map` 的功能）然后筛选出一部分（`filter` 的功能）。如果是先使用map然后filter的话，你需要遍历这个数组两次。
...
如果有了解最新 `ES` 的同学，相当于我们手动实现了 `Array.flat()` 方法。
