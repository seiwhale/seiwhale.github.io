---
title: Taro 开发说明与注意事项
date: 2019-12-25 14:59:00
tags:
  - React
  - JSX
  - Taro
...
description: Taro 是一套遵循 React 语法规范的多端开发解决方案。但是在实际的开发过程中与React还有一些不同的地方，我们都需要注意什么呢？
cover: https://cdn.jsdelivr.net/gh/seiwhale/assets/img/blog/taro.jpg
---

**Taro** 是一套遵循 [React](https://reactjs.org/) 语法规范的 **多端开发** 解决方案。但是在实际的开发过程中与 `React` 还有一些不同的地方，究竟在开发中有哪些需要注意的呢，下面就一起来看看吧。

## Taro 与 React 的差异

由于微信小程序的限制，`React` 中某些写法和特性在 `Taro` 中还未能实现，后续将会逐渐完善。

截止到本课程发布前，`Taro` 的最新版本为 `1.3`，因此以下讲解默认版本为 `1.3`。
...
`Taro` 函数命名使用**驼峰命名法**，如 `onClick`，由于微信小程序的 `WXM`L 不支持传递函数，函数名编译后会以字符串的形式绑定在 `WXML` 上，由于 `WXML` 的限制，函数名有三项限制：
...
![image](https://upload-images.jianshu.io/upload_images/14272773-e21dcadb460c1136.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![image](https://upload-images.jianshu.io/upload_images/14272773-fa01b2a89ad24626.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

![image](https://upload-images.jianshu.io/upload_images/14272773-e7e7e08a2fbcd48d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
由于 `JSX` 中的写法千变万化，我们不能支持到所有的 `JSX` 写法，同时由于微信小程序端的限制，也有部分 `JSX` 的优秀用法暂时不能得到很好地支持。这些不支持的写法都可以通过其他写法来规避，同时 `ESLint` 相关插件都能很好地提醒用户避免踩坑。
